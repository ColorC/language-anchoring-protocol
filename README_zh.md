...
> **⚠️ 实验性声明**：LAP 是一个早期设计框架。类型系统、语义关系模型和运行时原语都是正在 [OmniFactory](https://github.com/ColorC/omnifactory) 项目中验证的假设。本仓库仅包含**定义和参考代码**——生产实现和测试在 OmniFactory 中进行。
...
> **关于命名**：尽管仓库名中仍保留 "Protocol"，LAP 当前是一个**设计框架**（架构模式 + 参考原语），还不是互操作协议。升级为 "Protocol" 需要多方实现、一致性测试和线路格式——这些目前都不存在。
...
3.  **类型系统待验证**：语义关系模型（组合、合成、转换、继承）是一个假设。参考代码中只部分实现了继承和转换。完整的关系体系正在 OmniFactory 的 Evolution Engine 中验证。

4.  **规范与实现间隙**：V0.2 特性（`confidence`、`granted_tags`、标签传播、`required_tags` 检查）在 Pydantic 模型中定义了字段，但 `PipelineRunner` 和 `PipelineChecker` **尚未使用**。这些是等待 OmniFactory 验证的前向声明。

5.  **本仓库无测试**：本仓库包含定义和参考代码。测试和生产验证在 OmniFactory 项目中进行。
...
**LAP 的增量贡献**：提供了一套统一词汇（Anchor、Format、Verdict、Route）来横跨上述所有模式，并附带语义关系模型（组合、合成、转换、继承）和编译期检查。这种统一是否比直接使用上述工具提供了足够的实际价值，是 LAP 需要通过实际验证来回答的核心问题。
...
> **实现说明**：`python_impl/` 中的参考实现是与规范配套的概念验证。生产实现、验证和测试在 [OmniFactory](https://github.com/ColorC/omnifactory) 项目中进行。
