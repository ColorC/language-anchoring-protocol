...
```
PythonCode 属于 Code 类别
这意味着:
  流程中需要通用的 Code 时，传入 PythonCode 是绝对安全的（因为 PythonCode 是 Code 的一种）。
  但流程中指明要 PythonCode 时，传入一个通用的、不确定是什么语言的 Code 就会报错。
```

### 4.3 形态转换时的“丢包”风险与“自由发挥”

当数据在流水线中发生大变身（比如从一段需求变成几百行代码）时，必须明确使用转换引擎（Transformer）：

```
典型的形态演变流:
  需求 (Requirement) → 详细方案 (Spec)      (需求分析)
  详细方案 (Spec) → 代码 (Code)             (编程实现)
  代码 (Code) → 测试结果 (TestResult)       (跑测试)
```

**关键洞察：变身本质上是一次“信息坍缩”。** 
比如从需求到代码，那些感性的业务背景被丢弃，凝固成了死板的代码逻辑。

这里存在一个**“要不要听原话”的纠结**：通常我们希望生成的代码绝对忠实于需求。但如果你就是想要一个“爱发挥”的 Agent 来发散思维，那么这种偏离反而是你要的“忠实”。

LAP 的做法是：**不强制规定 AI 必须死板地翻译，但规定所有转换必须接受后续“检查站”的盘问。**

我们引入了 `confidence`（语义确信度）来量化这种转换的靠谱程度。如果一个 Agent 把需求翻译成了毫不相干的代码，虽然它符合“代码格式”，但它的“语义匹配度”会极低。在最后的检查中，它会被判定为 FAIL 并被打回重写。这意味着任何转换引擎后面，都应当紧跟着负责“质检”的锚点。
