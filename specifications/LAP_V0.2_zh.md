...
**它们是同一个模式的不同实例。** Design by Contract (Eiffel, 1986)、Guardrails AI、DSPy 等已有工作都触及了这一模式的不同方面。LAP 尝试将这些操作统一到一个带类型系统的框架下。
...
**每个工具解决了相似问题的不同切面。** LAP 的尝试是提供一层统一的类型化抽象，将上述工具各自覆盖的验证-重试-路由模式纳入一个带继承和编译期检查的框架。

MCP 标准化了输入侧（LLM 如何访问工具）。输出侧（LLM 产物如何被验证和路由）目前主要由上述各工具分别处理。LAP 探索在这一侧提供统一的设计模式。
...
### 10.3 长期探索：声明式 LLM 工作流设计

LAP 可以被视为一种面向 LLM 工作流的**声明式设计范式的探索**：
...
一个可能的长期方向是：用户声明起点和终点类型，系统自动推断中间所需的 Transformer 和 Anchor 链。这一愿景是否可行，取决于 LAP 的类型系统在实际场景中是否足够表达力，需要进一步实验验证。
