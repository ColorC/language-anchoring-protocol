...
### 2.6 发现六：语义类型需要语义维度 *(V0.2 新增)*

V0.1 的类型系统只有一个维度——继承链（`Code <: Spec <: Requirement`）。但现实中的语义有**多个正交维度**：

```
一份"代码差异" (code-diff) 可以是:
  - 来自 Git 的，或来自 SVN 的        → 来源维度
  - 关于前端的，或关于后端的            → 领域维度
  - 已审查的，或未审查的               → 状态维度
  - 安全相关的，或性能相关的            → 意图维度
```

**继承链只能表达"是什么"（is-a），不能表达"属于什么领域"、"处于什么状态"。** 如果把所有维度都编码到类型名里（`git-frontend-reviewed-security-code-diff`），类型系统会爆炸。

解决方案：**语义标签（Semantic Tags）**——在继承链之外引入可组合的标签维度。

```
Format.id    = 结构身份 (是什么)
Format.tags  = 语义维度 (属于什么、处于什么状态)
(id, tags)   = 完整的类型身份
```

### 2.7 发现七：未检验输入是类型安全的盲区 *(V0.2 新增)*

一个管线声称自己接受 `code-diff` 作为输入。但谁保证传进来的东西真的是 `code-diff`？

在 Rust 中，这个问题有明确答案：

```rust
// 未验证的原始输入 — 只是一个 String
struct RawInput(String);

// 验证后的类型 — 只能通过 validate() 构造
struct Email(String);
...
**在 LAP V0.1 中，这个保证不存在。** 任何数据都可以声称自己是任何 Format，管线会信以为真，直到某个下游节点因为数据不符预期而崩溃。

这是类型安全的最大盲区。V0.2 引入**入口守卫（Entry Guard）** 模式来解决它。
...
Format = {
    ...
    # V0.2 新增
    tags:                     语义标签列表 (该类型的固有语义维度)
    semantic_preconditions:   成为该类型的前置条件 (人类可读)
    required_tags:            输入数据必须已具备的标签 (机器可检查)
}
```
...
| 维度 | 表达方式 | 类比 | 用途 |
|------|---------|------|------|
| 结构身份 | `id` + `parent` 继承链 | Rust 的 `struct` | "这个东西的结构是什么" |
| 语义维度 | `tags` | Rust 的 `trait` | "这个东西属于什么语义空间" |

对**人类**：`id` 和 `name` 应当清晰描述类型的全部含义。
对**计算机**：`(id, tags)` 整体才是唯一标识，类型匹配检查两个维度。

#### 语义前置条件

`semantic_preconditions` 描述数据必须满足什么条件才有资格被称为这个类型：
...
format:
  ...
  semantic_preconditions:
    - "diff 已经过至少一位审查者的审阅"
    - "审查意见已附在 review_comments 字段中"
  required_tags: ["vcs-verified"]   # 输入必须先确认来自合法版本控制
```

`required_tags` 是**机器可检查的**——PipelineChecker 可以在编译期验证上游是否有 validator 授予了这些标签。`semantic_preconditions` 是**人类可读的**——指导 validator 实现者知道该检查什么。
...
| **Format（格式/类型）** | 流过管线的数据的语义类型。V0.2: 由 `(id, tags)` 共同定义完整身份。 |
| **Verdict（判定）** | Validator 的输出。`PASS / FAIL / PARTIAL` + 诊断信息。V0.2: 含 `confidence` 和 `granted_tags`。 |
...
| **Pipeline（管线）** | Anchor 和 Transformer 的有向类型安全组合。LAP 的"程序"。V0.2: 含 `group` 和 `tags`。 |
| **硬锚定（Hard Anchoring）** | 确定性判定。编译器、JSON Schema、测试套件。可证明。confidence = 1.0。 |
| **软锚定（Soft Anchoring）** | 概率性判定。LLM-as-judge、人工审查。不可证明。confidence < 1.0。 |
| **语义继承** | Format 之间的子类型关系。Code <: Spec <: Requirement。子类型保持父类型的意图语义但改变表达结构。 |
| **语义标签（Semantic Tags）** *(V0.2)* | Format 的正交语义维度。点分层级命名，支持缩窄/放宽/组合/交集。标签越多 = 类型越窄。 |
| **入口守卫（Entry Guard）** *(V0.2)* | 管线入口节点的设计模式。接受未检验输入，通过显式验证 narrow 为已检验类型。 |
| **语义距离（Semantic Distance）** *(V0.2)* | 输入数据的实际语义与 Format 声称的语义之间的差距。入口守卫的目标是将其压缩到 0。 |
| **授予标签（Granted Tags）** *(V0.2)* | Verdict 中声明的、由本次验证确认的语义维度。在管线中累积，只增不减。 |
| **语义置信度（Confidence）** *(V0.2)* | Verdict 中 0.0~1.0 的值，量化 validator 对语义匹配的确信程度。硬锚定器总是 1.0。 |
